...
function onJsonYouTube(json){
    console.log('JSON YouTube ricevuto');
    console.log(json);
    const risultati = document.querySelector('#risultati');
    risultati.innerHTML = '';
    const results_video = json.items;
    for(const result of results_video){
        const titolo = result.snippet.title;
        const video = document.createElement('iframe');
        video.src = 'https://youtube.com/embed/'+result.id.videoId;
        video.width = "420";
        video.height = "312";
...
function cerca(event){
    ...
    if(content){
        ...
        if(type === "film"){
            ...
            video_request = yt_api_endpoint + text + '&type=video&key=' + YT_APIKEY;
            fetch(video_request, requestOptions, {
                headers: {
                    'Content-Type': 'text/html; charset=UTF-8'
                }
            }).then(onResponse).then(onJsonYouTube);
        }
    } else {
...
// YOUTUBE API
const YT_APIKEY = 'AIzaSyDE3CaAiyKqkNLNRb3MzXqtAjfdgdj_qkA';
const yt_api_endpoint = 'https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=21&q=';
